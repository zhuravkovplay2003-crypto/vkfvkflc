...
function transformProductsData(productsData, variantsData) {
    ...
    productsData.forEach(productRow => {
        ...
        variants.forEach(variant => {
            ...
            // Ищем все колонки, которые не являются стандартными (ID товара, Название, Крепость, Сопротивление, Вкус, Цвет, URL фото)
            const standardColumns = ['ID товара', 'ID', 'Название', 'Крепость', 'Сопротивление', 'Вкус', 'Цвет', 'URL фото'];
            Object.keys(variant).forEach(columnName => {
                // Проверяем, что это не стандартная колонка и не пустая
                if (!standardColumns.includes(columnName) && columnName.trim() !== '') {
                    ...
                    console.log(`📍 Найдена точка самовывоза: "${locationName}" = ${quantity}`);
...
function init() {
    ...
    // Явно обновляем nav-right-content при инициализации, если мы на странице каталога
    // Это гарантирует, что кнопка выбора адреса отображается правильно
